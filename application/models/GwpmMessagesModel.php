 ...
 class GwpmMessagesModel {
	...
	function updateMessage($uid, $rid, $msg, $type, $pmid = null) {
		...
		if($type == 'int' && $pmid == null) {
			$queryString = "SELECT COUNT(MID) msgcount FROM " . $messages_table_name . " msgtab, " . $user_table_name . " usertab " .
			...
			if (isset ($obj) && $obj != null && $obj->msgcount > 0) {
				$endVal = $wpdb->update($messages_table_name, array("unread" => '1', "created" => $datetime ), array("pmid" => '-1', "user_id" => $uid, "recipient_id" => $rid)) ;
			} else {
